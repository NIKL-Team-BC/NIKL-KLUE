...
def train(model_dir, args):
  ...
  # load dataset
  train_dataset = load_data("./boolq_data_results/data/SKT_BoolQ_Train.tsv")
  val_dataset = load_data("./boolq_data_results/data/SKT_BoolQ_Dev.tsv")

  # train eval split 20% k-fold (5)
  datasets_ = load_data("./boolq_data_results/data/SKT_BoolQ_Train_All.tsv")
...
#   for i in range(4, 6):
  ...
  if args.wandb:
    wandb.login()
    wandb.init(project='NIKL-BoolQ', name=args.name, config=vars(args))
